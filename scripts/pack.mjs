...
const asarName = pkg.name.replaceAll(" ", "-");
const root = process.cwd();
const dist = path.resolve(root, "dist");
const tempPath = path.resolve(dist, "temp");
const asarPath = path.resolve(dist, asarName + ".asar");
...
async function main() {
    ...
    // Copy dist
    {
        ...
        for (const file of files) {
            ...
            if (await exists(filePath)) {
                ...
                console.warn(`${emojis.warn} Missing file ${file} in dist.`);
            }
        }
    }

    // Write git info 
    if (pkg.build.gitInfo) {
    ...
    console.log(`${emojis.success} Successfully packaged ${asarName}.asar`);
}
